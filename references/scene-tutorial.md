...
| 用途 | 变量名 | 色值 | 说明 |
|------|--------|------|------|
| 橙色 | `--yellow` | `#F07C1C` | 强调、装饰圆圈、连接线、badges |
| 柔黄 | `--yellow-soft` | `#FBE6D2` | 背景块、气泡底色 |
| 灰棕 | `--blue` | `#302C2B` | 英文标题、超链、重点标记 |
| 深灰棕 | `--blue-deep` | `#1F1B1A` | 大装饰字、section数字编号 |
...
10. **全宽品牌色面板**: 背景使用蓝/黄/橙纯色，文字反白。一页最多1~2个，用于打破奶白底的节奏。禁忌：不要在品牌色面板上放灰棕文字
...
- 选中文本高亮：`::selection { background: #F07C1C; color: #1a1a1a; }`
...
- **配色微调**: 内容跟某品牌相关时可替换灰棕为品牌色，但橙色+奶白底不变
- **布局组合**: 十一种布局模式自由组合
